...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   CREATE TYPE "public"."enum_announcements_status" AS ENUM('blocked', 'stopped', 'published', 'unpublished');
  ...
  ALTER TABLE "announcements" ADD COLUMN "author_id" integer NOT NULL;
  ALTER TABLE "announcements" ADD COLUMN "specialty_id" integer NOT NULL;
  ALTER TABLE "announcements" ADD COLUMN "status" "enum_announcements_status" DEFAULT 'unpublished' NOT NULL;
  ...
  ALTER TABLE "announcements" ADD COLUMN "work_time_days_sun" boolean;
