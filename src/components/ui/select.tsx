...
> = ({ children, className, ref, ...props }) => (
  <SelectPrimitive.Trigger
    className={cn(
      'flex h-10 w-full items-center justify-between rounded border border-input bg-background px-3 py-2 text-inherit ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-[#444] focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1',
      className,
    )}
    ref={ref}
    {...props}
  ...
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      className={cn(
        ...
        position === 'popper' &&
          'data-[side=bottom]:translateLabel-y-1 data-[side=left]:-translateLabel-x-1 data-[side=right]:translateLabel-x-1 data-[side=top]:-translateLabel-y-1',
        className,
      )}
      position={position}
      ref={ref}
